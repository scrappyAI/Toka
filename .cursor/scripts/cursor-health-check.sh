...
log() {
    ...
    echo "${timestamp} [$level] $message" >> "$CURSOR_LOG_DIR/health-check.log"
}
...
check_websocket_health() {
    ...
    # Use nc (netcat) to test WebSocket port
...
check_resource_usage() {
    ...
    # Check disk space
    local disk_available=$(df /app/data | tail -1 | awk '{print $4}')
...
check_database_health() {
    ...
    if [[ -f "/app/data/cursor-agents.db" ]]; then
        ...
        local db_check=$(sqlite3 /app/data/cursor-agents.db "SELECT 1;" 2>/dev/null || echo "ERROR")
        if [[ "$db_check" == "1" ]]; then
...
check_log_health() {
    ...
    for log_file in "${log_files[@]}"; do
        ...
        if [[ -f "$log_path" ]]; then
            ...
            local recent_errors=$(tail -n 100 "$log_path" | grep -i error | wc -l)
...
main() {
    log "INFO" "Starting cursor agent health check..."
